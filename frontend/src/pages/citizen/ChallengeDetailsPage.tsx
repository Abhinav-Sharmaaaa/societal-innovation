...
import type { TriageResult } from "../../services/aiService.ts";
...
const statusSteps = [
  ...
  },
  {
    key: "ROUTED",
    label: "Routed",
    description: "The challenge has been sent to the appropriate pathway.",
  },
  {
    key: "IN_PROGRESS",
    label: "In Progress",
...
export default function ChallengeDetailsPage() {
  ...
  const [aiAnalysis, setAiAnalysis] = useState<TriageResult | null>(
    navigationState?.aiAnalysis ?? null
  );
    ...
    return (
      <div className="challenge-details-page">
        <div className="details-container">
          <div className="details-loading">Loading challenge details...</div>
        </div>
      </div>
    );
  }
  ...
  const resolvedCategory = aiAnalysis?.category || challenge.category;
  const resolvedConfidence =
    aiAnalysis?.category_confidence ?? challenge.ai_confidence_score;
  const resolvedRouting = aiAnalysis?.routing_type || challenge.routing_type;
  const resolvedRoutingReason =
    aiAnalysis?.routing_reason || challenge.routing_reason;
    ...
    <div className="challenge-details-page">
      <div className="details-container">
        ...
        <div className="details-header">
          <div>
            <span className="details-eyebrow">CHALLENGE #{challenge.id}</span>
          ...
          <section className="details-card ai-summary-card">
            ...
            <div className="ai-summary-grid">
              <div className="ai-metric">
                ...
                <strong>{formatLabel(aiAnalysis.category)}</strong>
              </div>

              <div className="ai-metric">
                <span>Confidence</span>
                <strong>{percent(aiAnalysis.category_confidence)}</strong>
              </div>

              <div className="ai-metric">
                <span>Decision</span>
                <strong>{formatLabel(aiAnalysis.category_decision)}</strong>
              </div>

              <div className="ai-metric">
                <span>Routing</span>
                <strong>{formatLabel(aiAnalysis.routing_type)}</strong>
              </div>
            </div>
              ...
              <div className="ai-review-banner">
                ...
                <div>
                  ...
                  <p>
                    ...
                    so automated routing has been paused.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="ai-alternatives">
              ...
              {aiAnalysis.category_top_3.map((item) => (
                <div
            ...
            <div className="ai-details-row">
              ...
              <strong>{percent(Math.max(0, aiAnalysis.category_margin))}</strong>
            </div>

            <div className="ai-details-row">
              <span>Severity</span>
              <strong>{formatLabel(aiAnalysis.severity)}</strong>
            </div>

            <div className="ai-details-row">
              <span>Urgency</span>
              <strong>{formatLabel(aiAnalysis.urgency)}</strong>
            </div>

            <div className="ai-details-row">
              <span>Innovation Required</span>
              <strong>
                {aiAnalysis.innovation_required
                  ? `Yes (${percent(aiAnalysis.innovation_score)})`
                  : "No"}
              </strong>
            </div>

            <p className="ai-model-version">
              Model: {aiAnalysis.model_version}
            </p>
          </section>
        )}
          ...
          <main className="details-main">
                  ...
                  return (
                    ...
                    >
                      <div className="timeline-marker">
                        {completed ? <CheckCircle2 size={17} /> : <span />}
                      </div>
            ...
            <section className="details-card">
              ...
              {challenge.evidence.length === 0 ? (
                <div className="empty-evidence">No evidence has been uploaded.</div>
              ) : (
          ...
          <aside className="details-sidebar">
            <section className="details-card">
              ...
              <div className="info-list">
                <div className="info-row">
                  <span>Category</span>
                  <strong>{formatLabel(resolvedCategory)}</strong>
                </div>

                <div className="info-row">
                  <span>Urgency</span>
                  <strong className={`urgency-${challenge.urgency.toLowerCase()}`}>
                    {formatLabel(aiAnalysis?.urgency || challenge.urgency)}
                  </strong>
                </div>
                ...
                <div className="info-row">
                  <span>Severity</span>
                  <strong>
                    {formatLabel(aiAnalysis?.severity || challenge.severity)}
                  </strong>
                </div>
                ...
                <div className="info-row">
                  ...
                  <strong>
                    {challenge.affected_population ?? "Not specified"}
                  </strong>
                </div>
                ...
                <div className="info-row">
                  ...
                  <strong>
                    {challenge.estimated_economic_loss !== null
                      ? `₹${challenge.estimated_economic_loss.toLocaleString("en-IN")}`
                      : "Not specified"}
                  </strong>
                </div>
            ...
            <section className="details-card routing-card">
              ...
              <div className="routing-type">
                {formatLabel(resolvedRouting || "PENDING")}
              </div>
                ...
                resolvedConfidence !== undefined && (
                  <div className="confidence-block">
                    <div className="confidence-header">
                      ...
                      <strong>{percent(resolvedConfidence)}</strong>
                    </div>
                    ...
                    <div className="confidence-bar">
                      <div
                        ...
                        style={{
                          width: `${Math.min(
                            100,
                            Math.max(0, resolvedConfidence * 100)
                          )}%`,
                        }}
                      />
                    </div>
              ...
              {aiAnalysis && (
                <div className="confidence-block">
                  <div className="confidence-header">
                    ...
                    <strong>
                      {percent(Math.max(0, aiAnalysis.category_margin))}
                    </strong>
                  </div>
                  ...
                  <div className="confidence-bar">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          100,
                          Math.max(0, aiAnalysis.category_margin * 100)
                        )}%`,
                      }}
                    />
                  </div>
            ...
            <section className="details-card location-card">
              ...
              {challenge.address && <p>{challenge.address}</p>}

              <p>
                {challenge.district || "District not specified"}
                {challenge.state ? `, ${challenge.state}` : ""}
              </p>
